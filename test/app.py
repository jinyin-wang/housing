...
def random_sampling():
    ...
    df = random_samples.reindex(
        columns=['wage', 'educ', 'race', 'sex', 'hispanic', 'south', 'married', 'exper', 'union', 'age'])
...
def get_stratified_sampling():
    ...
    df_center = pd.DataFrame(center,
                             columns=['wage', 'educ', 'race', 'sex', 'hispanic', 'south', 'married', 'exper', 'union',
                                      'age'])
    # 标注每个点的聚类结果
...
data_csv = pd.read_csv('CPS85.csv', encoding='gbk')  # 读入数据
del data_csv['sector']
filters = ['wage', 'educ', 'race', 'sex', 'hispanic', 'south', 'married', 'exper', 'union', 'age']
...
data_csv_original = pd.read_csv('CPS85.csv', low_memory=False)

df_original = pd.read_csv('CPS85.csv')
data_original = df_original
data_drop = df_original.reindex(
    columns=['wage', 'educ', 'race', 'sex', 'hispanic', 'south', 'married', 'exper', 'union', 'age'])
data_old = pd.read_csv('output with index.csv')
data_old = data_old.reindex(
    columns=['wage', 'educ', 'race', 'sex', 'hispanic', 'south', 'married', 'exper', 'union', 'age'])
...
all_tran_data = data_drop[filters]
all_array_data = np.array(all_tran_data)
